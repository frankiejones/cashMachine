...
// pin enter function, 3 attepmts then locked out, takes you to option function if correct
...
const changePin =() => {
    let pinEntry = prompt ("Please enter your current pin to make changes, you have 3 attepmts before you are locked out")
    while (true) {
...
const withDraw = () => {
    ...
    if (withdraw <= balance && withdraw <= maxWithdraw) {
        ...
        alert(`You dont have available funds, you can only withdraw a max of £${balance}`);
        withDraw();
    };  
    optionMenu();
};
...
const depositAmount = () => {
    ...
    if (newDeposit > 250 ) {
        alert = ("You are only allowed to depoist £250 per day. Please enter a lower amount");
        depositAmount();
        ...
        alert("You have enteres an invalid entry, please try again");
        depositAmount();
    };
};
